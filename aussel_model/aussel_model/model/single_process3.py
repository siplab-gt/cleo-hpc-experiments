...
#    print('positions generated')
    ...
    if not preprocess_inputs:
        print("Skipping input preprocessing; assuming the files already contain firing rates")
    inputs1,inputs2,inputs3=apply_input(input_type,A0,A1,dur,f1,duty_cycle,runtime,in_file_1,in_file_2,in_file_3,in_fs, noise_adder=noise_adder, preprocess_inputs=preprocess_inputs)
...
#        print(all_rate_E_monitors)
        if plot_raster :
            all_spike_E_monitors=[[SpikeMonitor(Gpy) for Gpy in all_neuron_groups[i][0] if Gpy] for i in range(4)]
            all_spike_I_monitors=[[SpikeMonitor(Ginh) for Ginh in all_neuron_groups[i][1] if Ginh] for i in range(4)]
